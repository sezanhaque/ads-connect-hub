...
const MetaJobAds = () => {
  return (
    <div className="min-h-screen bg-background">
      ...
      {/* ─── HERO ─── */}
      <section className="container mx-auto px-4 pt-8 md:pt-14 pb-10 md:pb-12">
        <div className="grid lg:grid-cols-2 gap-8 lg:gap-12 items-center">
          ...
          <motion.div {...fadeUp} transition={{ duration: 0.45, delay: 0.12 }}>
            <div className="bg-gradient-to-br from-primary/5 via-accent/10 to-secondary/5 rounded-2xl p-5 md:p-6 shadow-xl">
      ...
      {/* ─── WHY META — Two column with visual ─── */}
          ...
          <div className="grid lg:grid-cols-2 gap-8 lg:gap-14 items-center">
            ...
            <motion.div {...fadeUp} transition={{ duration: 0.45, delay: 0.1 }}>
              <div className="grid grid-cols-2 gap-3">
                {/* Job board side */}
                <div className="bg-muted/60 rounded-xl border border-border/50 p-4 space-y-3">
                  <p className="text-xs font-now font-semibold text-muted-foreground uppercase tracking-wider">Job board</p>
                  <div className="space-y-2">
                    {["Active seekers only", "High competition", "Cost per listing"].map((t, i) => (
                      <div key={i} className="flex items-center gap-2 text-xs text-muted-foreground font-now">
                        <span className="w-4 h-4 rounded-full bg-destructive/15 flex items-center justify-center text-[10px]">✕</span>
                        {t}
                      </div>
                    ))}
                  </div>
                  <div className="bg-muted rounded-lg p-3 space-y-1.5 border border-border/30">
                    <div className="h-2 w-3/4 bg-border rounded" />
                    <div className="h-2 w-1/2 bg-border rounded" />
                    <div className="h-6 w-full bg-border/50 rounded mt-2" />
                  </div>
                  <p className="text-[10px] text-muted-foreground/60 font-now text-center">Limited reach</p>
                </div>

                {/* Social media side */}
                <div className="bg-gradient-to-br from-primary/5 to-accent/10 rounded-xl border border-primary/20 p-4 space-y-3 ring-1 ring-primary/10">
                  <p className="text-xs font-now font-semibold text-primary uppercase tracking-wider">Social feed</p>
                  <div className="space-y-2">
                    {["Passive candidates", "Targeted reach", "Cost per applicant"].map((t, i) => (
                      <div key={i} className="flex items-center gap-2 text-xs text-foreground font-now">
                        <span className="w-4 h-4 rounded-full bg-primary/15 flex items-center justify-center text-[10px] text-primary">✓</span>
                        {t}
                      </div>
                    ))}
                  </div>
                  <div className="bg-card rounded-lg border shadow-sm overflow-hidden">
                    <div className="flex items-center gap-1.5 px-2.5 py-1.5 border-b">
                      <div className="w-5 h-5 rounded-full bg-[#1877F2] flex items-center justify-center text-white text-[8px] font-bold">f</div>
                      <div>
                        <p className="text-[9px] font-now font-semibold text-foreground">Your Company</p>
                        <p className="text-[8px] text-muted-foreground font-now">Sponsored</p>
                      </div>
                    </div>
                    <div className="bg-gradient-to-br from-primary/15 to-accent/20 px-2.5 py-2 text-center">
                      <p className="text-[10px] font-now font-bold text-foreground">We're Hiring 🎯</p>
                      <p className="text-[8px] text-muted-foreground font-now">Developer · €65k+</p>
                    </div>
                    <div className="px-2.5 py-1.5 border-t">
                      <div className="h-4 w-full bg-primary/80 rounded text-[8px] text-primary-foreground font-now font-semibold flex items-center justify-center">Apply now</div>
                    </div>
                  </div>
                  <p className="text-[10px] text-primary/70 font-now text-center font-medium">3.5B+ daily users</p>
                </div>
              </div>
            </motion.div>
          </div>
      ...
      <section className="bg-muted/40 py-10 md:py-14">
        <div className="container mx-auto px-4">
          <motion.div className="text-center mb-8 space-y-2" {...fadeUp}>
            <h2 className="text-2xl md:text-3xl font-now font-bold text-foreground">How it works</h2>
            <p className="text-muted-foreground font-now">From vacancy to applicant in three steps.</p>
          </motion.div>
          ...
          <div className="grid md:grid-cols-3 gap-5 max-w-5xl mx-auto">
            {[
              ...
              },
              {
                ...
                step: "02",
                title: "Create & Launch",
                desc: "Choose between image or video ad formats — or let our platform generate them for you using AI. Our guided flow handles targeting, budget, and creative so you can launch in minutes.",
              },
              {
                icon: BarChart3,
                step: "03",
              ...
              <motion.div
                key={i}
                className="bg-card rounded-xl border p-5 space-y-3 shadow-sm"
                {...fadeUp}
      ...
      {/* ─── MERGED: BENEFITS + PLATFORMS ─── */}
      <section className="py-10 md:py-14">
        <div className="container mx-auto px-4">
          <motion.div className="text-center mb-8 space-y-2" {...fadeUp}>
            <h2 className="text-2xl md:text-3xl font-now font-bold text-foreground">Everything you need to run recruitment campaigns on Meta</h2>
          </motion.div>

          {/* Benefit cards — 3 top, 2 bottom */}
          <div className="max-w-5xl mx-auto space-y-5">
            <div className="grid md:grid-cols-3 gap-4">
              {[
                ...
                },
                {
                  ...
                  desc: "No designer? No problem. Our platform generates scroll-stopping ad creatives for you — both static images and video — tailored to your vacancy.",
                },
                {
                  icon: Film,
                ...
                <motion.div key={i} className="bg-card rounded-xl border p-5 space-y-2.5 shadow-sm" {...fadeUp} transition={{ duration: 0.45, delay: i * 0.06 }}>
                  <div className="inline-flex items-center justify-center w-10 h-10 rounded-lg bg-primary/10">
                    <b.icon className="h-5 w-5 text-primary" />
                  </div>
                  <h3 className="text-base font-now font-bold text-foreground">{b.title}</h3>
            ...
            {/* Platform cards — directly below, same section */}
            <div className="grid md:grid-cols-2 gap-4 max-w-4xl mx-auto pt-2">
              {/* Facebook */}
              <motion.div className="bg-muted/40 rounded-xl border p-5 space-y-3 shadow-sm" {...fadeUp}>
                <div className="flex items-center gap-3">
                  ...
                  <h3 className="text-lg font-now font-bold text-foreground">Facebook</h3>
                </div>
              ...
              {/* Instagram */}
              <motion.div className="bg-muted/40 rounded-xl border p-5 space-y-3 shadow-sm" {...fadeUp} transition={{ duration: 0.45, delay: 0.08 }}>
                <div className="flex items-center gap-3">
                  <div className="w-10 h-10 rounded-lg bg-gradient-to-br from-[#F58529]/10 via-[#DD2A7B]/10 to-[#8134AF]/10 flex items-center justify-center">
                  ...
                  <h3 className="text-lg font-now font-bold text-foreground">Instagram</h3>
                </div>
                <p className="text-sm text-muted-foreground font-now leading-relaxed">
      ...
      {/* ─── CTA — Full width ─── */}
      <section className="bg-gradient-to-br from-primary/10 via-accent/5 to-primary/8 border-y">
        <div className="container mx-auto px-4 py-12 md:py-16">
          <motion.div className="max-w-3xl mx-auto text-center space-y-5" {...fadeUp}>
